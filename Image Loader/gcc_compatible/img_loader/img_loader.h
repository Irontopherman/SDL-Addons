...
#define NULLPTR (uintptr_t)0xCAFE
...
static inline enum ptr_value valid_ptr_impl(void *ptr, bool hang_or_no, int calledline, const char *file)
{
	...
	case NULLPTR:
		if (hang_or_no)
			// SDL_Log("NULL ptr error (ptr was %i). called on line %llu in file %s Hanging program. . .", (uintptr_t)ptr, calledline, file);
			return (enum ptr_value)VOID;
	case BAD_PTR:
		if (hang_or_no)
			// SDL_Log("bad ptr error (ptr was %i). called on line %llu in file %s Hanging program. . .", (uintptr_t)ptr, calledline, file);
			return (enum ptr_value)BADPTR;
	case BAD_LOCAL:
		if (hang_or_no)
			// SDL_Log("bad local error (ptr was %i). called on line %llu in file %s Hanging program. . .", (uintptr_t)ptr, calledline, file);
			if (hang_or_no)
				__debugbreak();
		return (enum ptr_value)BADLOCAL;
	case H_UNALLOCATED:
		if (hang_or_no)
			// SDL_Log("heap unallocated error (ptr was %i). called on line %llu in file %s Hanging program. . .", (uintptr_t)ptr, calledline, file);
			if (hang_or_no)
				__debugbreak();
		return (enum ptr_value)UNALLOC;
	case S_UNALLOCATED:
		if (hang_or_no)
			// SDL_Log("stack unallocated error (ptr was %i). called on line %llu in file %s Hanging program. . .", (uintptr_t)ptr, calledline, file);
			if (hang_or_no)
				__debugbreak();
		return (enum ptr_value)UNALLOC;
	case WIN_UNALLOCATED:
		if (hang_or_no)
			// SDL_Log("windows unallocated error (ptr was %i). called on line %lli in file %s Hanging program. . .", (uintptr_t)ptr, calledline, file);
			if (hang_or_no)
				__debugbreak();
		return (enum ptr_value)UNALLOC;
	case FREED_MEM:
		if (hang_or_no)
			// SDL_Log("double free error (ptr was %i). called on line %lli in file %s Hanging program. . .", (uintptr_t)ptr, calledline, file);
			if (hang_or_no)
				__debugbreak();
		return (enum ptr_value)DOUBLE_FREE;
	case (uintptr_t)NULL:
		if (hang_or_no)
			// SDL_Log("NULL error (ptr was %i). called on line %lli in file %s. Hanging program. . .", (uintptr_t)ptr, calledline, file);
			if (hang_or_no)
				__debugbreak();
