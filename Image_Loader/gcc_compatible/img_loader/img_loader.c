...
{
    char *buffer = SDL_malloc(2048 * sizeof(char));
    size_t buffsize = (2048 * sizeof(char));
    buffer = SDL_GetCurrentDirectory();
    SDL_strlcat(buffer, "images\\", buffsize);
    // SDL_Log(target->imgname);
    SDL_strlcat(buffer, target->imgname, buffsize);
    SDL_strlcpy(path, buffer, pathSize);
    SDL_free(buffer);
    // SDL_Log("dir finshed, result:%s", path);
}
void create_img(image *targetimg, const char *imgname, double x, double y, double angle, double scale)
{
    targetimg->x = x;
    targetimg->y = y;
    targetimg->imgname = (char *)imgname;
    ...
    targetimg->flipped = false;
    targetimg->flipstate = SDL_FLIP_NONE;
...
void setWD(image *target, SDL_Renderer *renderer)
{
    ...
    {
        SDL_Log("Image not loaded, imgsize was NULL pointer");
        return;
    }
...
int maketexture(image *target, SDL_Renderer *renderer)
{
    ...
    if (target->flipped)
        target->flipstate = SDL_FLIP_HORIZONTAL;
    else
        target->flipstate = SDL_FLIP_NONE;
    if (target->textloaded)
        target->textloaded = false;
...
void draw(image *target, SDL_Renderer *renderer)
{
    ...
    SDL_RenderTextureRotated(renderer, target->rendertextture, NULL, &target->coords, target->angle, &target->imgcenter, target->flipstate);
}
void benchdraw(SDL_Renderer *renderer, image *thingtodraw)
{
    ...
    {
        dst.x = (float)(SDL_rand(800));
        dst.y = (float)(SDL_rand(600));
        double angle = SDL_rand(360);
        float scale = SDL_rand(10);
...
void resetbatch(const char *batchname)
{
    for (int allbats = numofbatches - 1; allbats >= 0; allbats--)
    {
        ...
        */
        ...
        if (batchname == NULL)
        {
            SDL_Log("bathcname was nul in resetbatch");
            continue;
        }
        if (allbatches[allbats].batch_imgname == NULL)
        {
            if (DEBUGLOGS)
                SDL_Log("batch_imgname is nul, continuing. . .");
            continue;
        }
        if (valid_ptr((char *)allbatches[allbats].batch_imgname, false) != FINE)
            continue;
        ...
        {
            SDL_free(allbatches[allbats].indices);
            SDL_free(allbatches[allbats].vertices);
            SDL_free((char *)allbatches[allbats].batch_imgname);
            if (DEBUGLOGS)
                SDL_Log("FREED BATCH %s", batchname);
...
void make_batch_texture(batch *target, SDL_Renderer *ren)
{
    if (target->batch_texture == NULL) // POSSIBLE MEMORY LEAK-----POSSIBLE MEMORY LEAK-----POSSIBLE MEMORY LEAK-----POSSIBLE MEMORY LEAK-----POSSIBLE MEMORY LEAK-----POSSIBLE MEMORY LEAK-----POSSIBLE MEMORY LEAK-----
    {
        ...
        */
        ...
        SDL_strlcat(buffer, "images\\", buffsize);
        // SDL_Log(target->imgname);
        SDL_strlcat(buffer, target->batch_imgname, buffsize);
        SDL_strlcpy(fillablepath, buffer, pathlen);
        SDL_free(buffer);
        // SDL_Log("dir finshed, result:%s", path);
...
void addnewbatch(image *target, batch **assignedbatch, SDL_Renderer *ren)
{
    ...
    batch *tempalloc = SDL_realloc(allbatches, (numofbatches + 1) * sizeof(batch));
    if (valid_ptr(tempalloc, true) != FINE)
    {
        SDL_Log("could not allocate new batch");
        // free(tempalloc);
        return;
    }
    allbatches = tempalloc;
    memset(&allbatches[numofbatches], 0, sizeof(batch));
    if (DEBUGLOGS)
        SDL_Log("batch count:%lli", numofbatches);
    *assignedbatch = &allbatches[numofbatches];
    ...
    tempindice = SDL_realloc((*assignedbatch)->indices, 6 * sizeof(int));

    if (tempindice == NULL)
    {
        SDL_Log("malloc failed for indices in function add new batch");
        return;
    }

    tempvertex = SDL_realloc((*assignedbatch)->vertices, 6 * sizeof(SDL_Vertex));
    if (tempvertex == NULL)
    {
        SDL_Log("malloc failed for vertices in function add new batch");
        return;
    }
    ...
    {
        SDL_free((char *)(*assignedbatch)->batch_imgname);
        (*assignedbatch)->batch_imgname = NULL;
        SDL_Log("Failed to strdup batch_imgname");
        return;
    }
    if (DEBUGLOGS)
        SDL_Log("setting vertexspot and indicespot to 0, prevous values vrtspt:%i, indcspot:%i",
    ...
    */
    ...
    else
    {
        ...
        for (int64_t checkimg = numofbatches - 1; checkimg >= 0; checkimg--)
        {
            ...
            if (valid_ptr((char *)allbatches[checkimg].batch_imgname, false) != FINE)
            {
                addnewbatch(target, &assignedbatch, ren);
                if (DEBUGLOGS)
                    SDL_Log("in addtobatch .batchimgname was null");
                break;
            }
            ...
            */
            ...
            if (match == 0 || lastmatch)
            {
                ...
                */
                ...
                int *temppointer = SDL_realloc(
                    assignedbatch->indices,
                    (assignedbatch->indicespot + (int64_t)6) * sizeof(int));
                if (temppointer == NULL)
                {
                    SDL_Log("could not allocate temppointer in function addtobatch");
                    return;
                }
                assignedbatch->indices = temppointer;
                // temppointer = NULL;
                SDL_Vertex *tempvertex = SDL_realloc(
                    assignedbatch->vertices,
                    (assignedbatch->vertexspot + (int64_t)6) * sizeof(SDL_Vertex));
                if (tempvertex == NULL)
                {
                    SDL_Log("could not allocate tempvertex in function addtobatch");
                    return;
                }
                assignedbatch->vertices = tempvertex;
                assignedbatch->vertexspace += 6;
    ...
    if (assignedbatch->vertexspot == 3722304989)
    {
        ...
        if (DEBUGLOGS)
            SDL_Log("assignedbatch->vertexspot was at max unsigned int");
    }
    ...
    {
        if (DEBUGLOGS)
            SDL_Log("assignedbatch verticies is null in addtobatch");
        return;
    }
...
void freebatches()
{
    ...
    // numofbatches = 0;
    if (DEBUGLOGS)
        SDL_Log("FREED ALL BATCHES, LEAKED POINTERS:");
